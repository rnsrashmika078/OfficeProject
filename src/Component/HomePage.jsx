      ...
      <div className="container text-center mt-5">
        ...
        <div className="d-flex justify-content-center gap-4 mt-4">
          {localStorage.getItem('adminlogtoken') > 0 ? (
            ...
            <Link to="/admin">
              <button className="btn btn-primary btn-lg">{localStorage.removeItem('userlogtoken')}{localStorage.removeItem('adminlogtoken')}{localStorage.removeItem('username')}ADMIN</button>
            </Link>
          )}
           {localStorage.getItem('userlogtoken') > 0 ? (
            ...
            <Link to="/user">
            <button className="btn btn-secondary btn-lg">{localStorage.removeItem('userlogtoken')}{localStorage.removeItem('adminlogtoken')}{localStorage.removeItem('username')}USER</button>
          </Link>
          )}
        </div>
        <div className="mt-5">
